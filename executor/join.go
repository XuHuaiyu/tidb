...
type HashJoinExec struct {
	...
	// case radixBits will be 1, we can skip the data phase.
	radixBits       uint32
...
// ptr4Partition stores the actual index in HashJoinExec.part for every rows of
// innerResults.
...
func (e *HashJoinExec) partitionInnerRows() {
	if !e.evalRadixBit() {
		return
	}
	e.preAlloc4InnerParts()

	wg := &sync.WaitGroup{}
	defer wg.Wait()
	for i := 0; i < e.partConcurrency; i++ {
		go util.WithRecovery(func() { e.doInnerPartition(i, wg) }, nil)
	}
	wg.Add(e.partConcurrency)
}
...
func (e *HashJoinExec) doInnerPartition(startIdx int, wg *sync.WaitGroup) {
	...
	for ; chkIdx < chkNum; chkIdx += e.partConcurrency {
		...
		for rowIdx, partPtr := range e.innerRowPrts[chkIdx] {
			...
			partition := e.innerParts[partIdx]
			partition.Insert(rowPtr, chk.GetRow(rowIdx))
		}
	}
}
...
func (e *HashJoinExec) getPartition(idx uint32) (part partition) {
	part = e.innerParts[idx]
	if part == nil {
		part = chunk.NewList(e.innerExec.retTypes(), e.initCap, e.maxChunkSize)
	}
	return
}
...
func (e *HashJoinExec) evalRadixBit() (needPartition bool) {
	...
	radixBitsNum := uint(math.Log2(innerResultSize / l2CacheSize))
	e.radixBits = ^(math.MaxUint32 >> radixBitsNum)
	if e.radixBits <= 1 {
		return false
	}
	e.innerParts = make([]partition, radixBitsNum)
	return true
}
